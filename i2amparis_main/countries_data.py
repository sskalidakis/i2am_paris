...
class RetriveDB:
    def __init__(self, model_name):
        ...
        if self.model_name != "":
            ...
            self.model_id = ModelsInfo.objects.all()[0]
    ...
    def create_json(self):
      ...
      """
        ...
        regions = Regions.objects.filter(model_name=model_id)
        data = []
        ...
        for k, region in enumerate(regions):
              ...
              countries = Countries.objects.filter(model_name=model_id, region=region_id)
    ...
    def retrieve_sectors(self):
        ...
        for i in subcategories:
            ...
            if self.model_name == '':
                ...
                temp_lst = list(map(lambda x: {x: Sectors.objects.filter(name=x, subcategory=temp, model_name=self.model_id).count() > 0}, temp_lst))
        ...
        for i in categories:
            ...
            if self.model_name == '':
                ...
                temp_lst = list(map(lambda x: {x: Sectors.objects.filter(name=x, category=temp, model_name=self.model_id).count() > 0}, temp_lst))
        ...
        for i in categories2:
            ...
            for j in subcategories:
                ...
                if self.model_name == '':
                    ...
                    temp_lst = list(map(lambda x: {x: Sectors.objects.filter(subcategory=j, name=x, model_name=self.model_id).count() > 0 }, temp_lst))
    ...
    def retrieve_emission(self):
        ...
        for i in categories:
            ...
            temp = list(map(lambda x: '<li> <font color = "{}" >{} </font> </li> '.format(self.bool_dict[x[1]], x[0]),temp))
            temp = '<ul> {} </ul>'.format(''.join(temp))
    ...
    def retrieve_mitigation_adaption(self):
        ...
        behavior_lst =  list(filter(lambda x: 'BEHAVIOUR' in x.upper(),list(Mitigations.objects.values_list('subcategory',flat=True).distinct())))
        ...
        for i in categories_mitgation[:3]:
            ...
            for j in temp_sub:
                temp_all = list(Mitigations.objects.filter(subcategory=j, category=i).values_list('name', flat=True).distinct())
                ...
                mitigation_adaption_dict.update({
                    j: self.is_enable_category(self.create_html_lists(list(map(lambda x: {x: True} if x in temp_true else {x: False}, temp_all))))
                })
        for i in categories_mitgation[3:-1]:
            sub_categories = list(Mitigations.objects.filter(category=i).values_list('subcategory', flat=True).distinct())
        ...
        temp_true = list(Mitigations.objects.filter(category='LULUCF', model_name=self.model_id)
                         .values_list('name', flat=True).distinct())
        mitigation_adaption_dict.update({
            'LULUCF': self.is_enable_category(self.create_html_lists(list(map(lambda x: {x: True} if x in temp_true else {x: False}, temp_all))))
        })
    ...
    def create_models_btn(self):
        ...
        return models_names
        # return " ".join(list(map(lambda x: '<a href="{}" >{}</a>'.format('dynamic_doc/'+x, x),  models_names)))
