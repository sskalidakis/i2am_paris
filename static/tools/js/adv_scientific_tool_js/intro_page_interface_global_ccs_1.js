$(document).ready(function () {
    ...
    $("#global_ccs_1-clear-button").click(function () {
        $('#global_ccs_1 select.sum-boot-select').multipleSelect('setSelects', []);
        $('#global_ccs_1_viz_frame_div').hide();
    });

    $("#global_ccs_1-run-button").click(function () {
        var viz_frame = $('#global_ccs_1_viz_frame_div');
        var model_sel = $('#global_ccs_1_model_name');
        var scenario_sel = $('#global_ccs_1_scenario_name');
        ...
        if (model_full || scenario_full) {
            ...
            /* # Query creation*/
            var jq_obj = create_global_ccs_1_query();
            console.log('Global CCS 1 JSON Query Created');
            retrieve_series_info_global_ccs_1(jq_obj);
    ...
    function start_qc_v_global_ccs_1_process(json_query_obj, variable){
        var query = {};
        query["query_name"] = "global_ccs_1_query";
        ...
        $.ajax({
            ...
            success: function (data) {
                console.log('Global CCS 1 Query Saved in DB');
    ...
    function create_visualisation_global_ccs_1(query_id, val_list, title_list, unit_list, variable) {
        var viz_frame = $('#global_ccs_1_viz_iframe');
        viz_frame.off();
        viz_frame.hide();
        $('#global_ccs_1_loading_bar').show();
        ...
        console.log('Global CCS 1 Ready to launch visualisation');
        var complete_url = "/visualiser/show_line_chart?" + url;
        viz_frame.attr('src', complete_url);
        viz_frame.on('load', function () {
            console.log('Global CCS 1 Visualisation Completed');
            $(this).show();
            $.ajax({
                ...
                contentType: 'application/json',
                success: function (data) {
                    console.log("Global CCS 1 Temporary Query Deleted");
                },
                error: function (data) {
                    console.log(data);
                }
            ...
            $('#global_ccs_1_loading_bar').hide();

        });

    }

    function create_global_ccs_1_query() {
        var sel_model = $('#global_ccs_1_model_name');
        var sel_scenario = $('#global_ccs_1_scenario_name');
    ...
    $("#global_ccs_1-run-button").trigger('click');
});
